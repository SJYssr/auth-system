...
| 变量 | 说明 |
|---|---|
| `CORS_ORIGINS` | 额外允许跨域的来源（逗号分隔） |
| `TRUST_PROXY` | 信任的反代层数，默认 `1`；进程直接暴露公网务必设 `false` |
| `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASS` | SMTP 服务（三项 host/user/pass 同时配置后启用邮件功能） |
| `SMTP_FROM` / `SMTP_SECURE` | 发件人（默认 SMTP_USER）；是否隐式 TLS（默认按端口 465 判断） |
| `ADMIN_EXPIRY_REMIND_DAYS` | 管理员到期前 N 天开始邮件提醒，默认 7，每 24 小时至多一次 |

## 多实例部署说明

限流计数（`rate_limits` 表）、图形验证码（`captchas` 表）、Webhook 投递队列（`webhook_deliveries` 表）均存储在 MySQL 中，可水平扩展多个应用副本共享状态：

- 限流阈值按「所有副本合计」生效；存储故障时限流 fail-open 放行，不影响正常请求。
- Webhook 投递失败按 30s→1m→5m→30m→1h 自动重试（最多 6 次尝试），重试扫描用 `FOR UPDATE ... SKIP LOCKED` 认领，多副本不会重复投递；投递记录保留 14 天。
- 到期邮件提醒同样以原子 UPDATE 认领，多副本不会重复发送。
...
> 特别注意：升级到 Token 哈希存储的版本（v1.0.0）后，所有管理员的旧会话一次性失效，需重新登录，属预期行为；升级到 v1.2.0 后，v1.1.0 时代签发的存量**卡密**明文会话同样一次性失效（哈希兼容比对已移除），客户端重新登录即可。
